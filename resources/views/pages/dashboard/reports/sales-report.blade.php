                                        ...
                                        <div class="border-b ns-box-body">
                                            <table class="table ns-table w-full">
                                                <tbody class="text-primary">
                                                    <tr class="">
                                                        <td width="200"
                                                            class="font-semibold p-2 border text-left bg-info-secondary border-info-primary text-white">
                                                            {{ __('Sub Total') }}</td>
                                                        <td class="p-2 border text-right border-info-primary">
                                                            @{{ summary.subtotal | currency }}</td>
                                                    </tr>
                                                    <tr class="">
                                                        <td width="200"
                                                            class="font-semibold p-2 border text-left bg-error-secondary border-error-primary text-white">
                                                            {{ __('Sales Discounts') }}</td>
                                                        <td class="p-2 border text-right border-error-primary">
                                                            @{{ summary.sales_discounts | currency }}</td>
                                                    </tr>
                                                    <tr class="">
                                                        <td width="200"
                                                            class="font-semibold p-2 border text-left bg-error-secondary border-error-primary text-white">
                                                            {{ __('Sales Taxes') }}</td>
                                                        <td class="p-2 border text-right border-error-primary">
                                                            @{{ summary.sales_taxes | currency }}</td>
                                                    </tr>
                                                    <tr class="">
                                                        <td width="200"
                                                            class="font-semibold p-2 border text-left bg-info-secondary  text-white">
                                                            {{ __('Shipping') }}</td>
                                                        <td class="p-2 border text-right border-success-primary">
                                                            @{{ summary.shipping | currency }}</td>
                                                    </tr>
                                                    <tr class="">
                                                        <td width="200"
                                                            class="font-semibold p-2 border text-left bg-success-secondary border-success-secondary text-white">
                                                            {{ __('Total') }}</td>
                                                        <td class="p-2 border text-right border-success-primary">
                                                            @{{ summary.total | currency }}</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                ...
                                <table class="table ns-table w-full">
                                    <thead class="text-primary">
                                        <tr>
                                            ...
                                            <th width="150" class="border p-2">{{ __('Units') }}</th>
                                ...
                                <table class="table ns-table w-full">
                                    <thead class="text-primary">
                                        <tr>
                                            ...
                                            <th width="100" class="border p-2">{{ __('Units') }}</th>
                                    ...
                                    <tbody class="text-primary">
                                        <template v-for="(category, categoryIndex) of result">
                                            <template v-if="category.products.length > 0">
                                                <tr v-for="(product,productIndex) of category.products"
                                                    :key="parseInt(category.id + '' + product.id)">
                                            ...
                                            <tr :key="categoryIndex" class="bg-info-primary">
                                                <td colspan="2" class="p-2 border border-info-primary">
                                                    @{{ category.name }}</td>
                                                <td class="p-2 border border-info-primary">
                                                    @{{ category.products.unit_name }}</td>
                                                <td class="p-2 border text-right border-info-primary">
                                                    @{{ computeTotal(category.products, 'quantity') }}</td>
                                                <td class="p-2 border text-right border-info-primary">
                                                    @{{ computeTotal(category.products, 'discount') | currency }}</td>
                                                <td class="p-2 border text-right border-info-primary">
                                                    @{{ computeTotal(category.products, 'tax_value') | currency }}</td>
                                                <td class="p-2 border text-right border-info-primary">
                                                    @{{ computeTotal(category.products, 'total_price') | currency }}</td>
                                            </tr>
                                        </template>
                                    </tbody>
                                    <tfoot class="text-primary font-semibold">
                                        <tr>
                                            <td colspan="3" class="p-2 border text-primary"></td>
                                    ...
                                    <tbody class="text-primary">
                                        <template v-for="(category, categoryIndex) of result">
                                            <tr :key="categoryIndex" class="">
                                                <td class="p-2 border text-left border-info-primary">
                                                    @{{ category.name }}</td>
                                                <td class="p-2 border text-right border-info-primary">
                                                    @{{ computeTotal(category.products, 'quantity') }}</td>
                                                <td class="p-2 border text-right border-info-primary">
